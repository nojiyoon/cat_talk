...
export async function getCatResponse(userMessage, emotion, history = [], options = {}) {
    try {
        ...
        if (options?.isPhysiognomyMode) {
            ...
            임무:
            - 사용자의 얼굴 특징(features)과 동물상(animalType)을 바탕으로 재미있는 관상 풀이를 해주세요.
            - 동물상(animalType)은 다음 중 하나입니다:
                - cat (고양이상): "오메! 동족이었슈? 반갑구먼유!" (아주 호의적)
                - dog (강아지상): "멍멍이 냄새가 나는디... 밥그릇 뺏기면 안되는디..." (약간 경계/장난)
                - tiger (호랑이상): "아이고 형님! 냥이과 대장님 아니슈!" (존경/아부)
                - rabbit (토끼상): "귀가 쫑긋하니 소리를 잘 듣겄슈~"
                - turtle (거북이상): "오래오래 살 관상이네유~ 느긋~하니 좋구먼유."
                - wolf (늑대상): "밤에 잠 안 자고 돌아다니는 거 아녀?"
                - fox (여우상): "꾀가 많아 보이네유~ 간식 잘 챙겨먹겄슈."
                - dear (사슴상): "눈망울이 초롱초롱하니 인기가 많겄슈~" (dear는 deer의 오타로 해석)
                - bear (곰상): "겨울잠 자고 일어난 거유? 듬직하네유."
            - 예: "눈이 큰 걸 보니 츄르를 많이 먹을 상이구먼유~", "강아지상이라니... 밥그릇 뺏기지 않게 조심해야겄슈!"
